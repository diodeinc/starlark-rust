...
// the struct packs into 8 usizes. On 32-bit platforms, fixed-size fields and
// target-specific Mutex layouts do not scale uniformly with usize. Keep the
// wasm32 layout within its established 12-usize budget without requiring an
// exact size, and do not assert a fixed size for other 32-bit targets.
